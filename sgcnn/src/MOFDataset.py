...
class MOFDataset():
    ...
    def __init__(self,
                 save_file_name,
                 set_dir="/data/mofdatabase_big",
                 label="Henry's constant: mol/(m^3 Pa)"):
    ...
    def save_data(self):
        set_data = self.get_data(self.set_dir)

        print("Finished generating training data")
        data_file_name = "dataset_" + self.save_file_name + ".p"

        data_file = open(data_file_name, "wb")
        pickle.dump(set_data, data_file)
    ...
    def get_data_helper(self, structure, y):
        ...
        # data.weight = data.weight.unsqueeze(dim=1).type(torch.FloatTensor)
        data.weight = torch.stack((data.weight, data.weight, data.weight,data.weight, data.weight, data.weight, data.weight, data.weight, data.weight, data.weight, data.weight), dim=1)
    ...
    def get_data(self, data_dir):
        ...
        size = len(labels['Name_MOF_CoRE_1272'])

        print(size)

        counters = list(range(size))

        directory_list = [directory + file + ".cif" for file in labels['Name_MOF_CoRE_1272'].values]
